...
class MailMCPServer {
      ...
      return {
        tools: [
          ...
          },
          {
            ...
            description: 'Get unseen (unread) messages. Use headers_only=true to fetch 30+ messages without hitting the 1MB response cap.',
            inputSchema: {
              type: 'object',
              properties: {
                limit: {
                ...
                headers_only: {
                  type: 'boolean',
                  description: 'If true, return only metadata (date/from/to/subject/flags) — no body/HTML/attachments. Required for listing many messages.',
                  default: false,
                },
                max_body_length: {
                  type: 'number',
                  description: 'Truncate text body to this many chars (0 = strip body, omit/negative = full body). Ignored if headers_only=true.',
                },
              },
            },
          },
          {
            ...
            description: 'Get recent messages from INBOX. Use headers_only=true to fetch 30+ messages without hitting the 1MB response cap.',
            inputSchema: {
              type: 'object',
              properties: {
                limit: {
        ...
        switch (name) {
          ...
          case 'get_unseen_messages':
            return await this.handleGetUnseenMessages(
              ...
              args?.headers_only as boolean | undefined,
              args?.max_body_length as number | undefined,
            );
  ...
  private async handleGetUnseenMessages(limit?: number, headersOnly?: boolean, maxBodyLength?: number) {
    await this.ensureRequiredConnections(true, false);
  ...
  private async handleGetRecentMessages(limit?: number, headersOnly?: boolean, maxBodyLength?: number) {
    await this.ensureRequiredConnections(true, false);
